...
void initialize_filesystem_sdmmc2() {
    ...
    esp_err_t init_vfs_fat_sdmmc = esp_vfs_fat_sdmmc_mount(mount_point, &host, &slot_config, &mount_config, &card);
    if (init_vfs_fat_sdmmc != ESP_OK)
    {
        ESP_LOGE("SD", "Failed to mount SDMMC (%s)", esp_err_to_name(init_vfs_fat_sdmmc));
        return;
    }
